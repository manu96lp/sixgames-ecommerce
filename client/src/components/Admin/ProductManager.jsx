...
function ProductManager( )
{
	...
	const handleEditClick = ( event ) => {
		...
		history.push( `/admin/products/${ value }`, { search: location.search } );
	};
	
	const handleDeleteClick = ( event ) => {
		const value = parseInt( event.target.value );
		
		productApi.deleteProductById( value )
			.then( ( response ) => {
				...
				dispatch( showNotification( "¡Producto eliminado correctamente!", "success" ) );
			} )
			.catch( ( error ) => {
				dispatch( showNotification( "¡Ocurrió un error al intentar eliminar el producto!", "error" ) );
			} );
	};
	...
	useEffect( ( ) => {
		...
		history.push( `/admin/products?${ query }` );
			...
			<div className="admin__page-panel">
				<Row>
					...
					<Col className="admin__page-panel--action">
						<Link to={ { pathname: "/admin/products/0", state: location.search } }>
							<button>
